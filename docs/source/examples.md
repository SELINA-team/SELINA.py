...
Running the command shown above will output five useful files in the res/preprocess folder.
...
- `3. query_single_expr.txt` : expression matrix of query data for the prediction step, the first column stores genes and the following columns store expression of all cells
...
This step will output eight files in the res/predict/example1 folder. Note that if the input is a cluster level matrix, then the last four files will not be generated.
...
- `7. example1_unknown_percent.txt` : the percentage of cells that are assigned unknown in each cluster, each row represents one cluster. Clusters with a higher unknown percentage may contain novel cell types that are not included in the reference
...
lower prediction probability, indicating that they are the malignant clusters or clusters with novel cell types.
